...
export const TripsPage: React.FC<TripsPageProps> = ({ language = 'fr', currentWeather }) => {
  ...
  const handleFindNearbyBusStops = async () => {
    ...
    try {
      ...
      if (flyOriginCoords) {
        ...
        if (coordsMatch) {
          ...
          if (data && data.length > 0) {
            ...
            throw new Error("Impossible de localiser l'adresse de départ saisie.");
          }
        } else {
          if (Capacitor.isNativePlatform()) {
            ...
            if (permissionStatus.location !== 'granted') {
              ...
              if (req.location !== 'granted') throw new Error('Permission de géolocalisation refusée.');
            }
      ...
      setBusStopError(`Échec Overpass : ${err.message || 'Problème réseau'}`);
      setNearbyBusStops([]);
    } finally {
  ...
  return (
    <div className="space-y-4 text-xs w-full max-w-xl mx-auto pb-10 px-1 font-sans text-slate-100">
        ...
        <div className="fixed inset-0 z-[99999] flex items-start justify-center pt-12 bg-black/85 backdrop-blur-xs p-4 animate-fade-in">
          <div className="bg-[#121622] border border-sky-400/80 rounded-2xl w-full max-w-lg p-5 shadow-2xl space-y-4 relative">
            <div className="flex items-center justify-between border-b border-slate-700 pb-3">
              ...
              <button type="button" onClick={() => setShowAddModal(false)} className="p-1.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-slate-300 hover:text-white cursor-pointer border border-slate-700">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <form onSubmit={handleAddOrUpdateTrip} className="space-y-3">
              <div className="space-y-1">
                <label className="text-[10px] text-slate-300 font-bold uppercase tracking-wide">Nom du trajet</label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 relative">
                <div className="space-y-1 relative">
                  <label className="text-[10px] text-slate-300 font-bold uppercase tracking-wide">Départ</label>
                ...
                <div className="space-y-1 relative">
                  <label className="text-[10px] text-slate-300 font-bold uppercase tracking-wide">Arrivée</label>
                  ...
                  {destinationSuggestions.length > 0 && (
                    <div className="absolute left-0 right-0 top-full mt-1 bg-[#050811] border border-teal-400 rounded-xl shadow-2xl z-[999999] overflow-hidden max-h-40 overflow-y-auto">
                      {destinationSuggestions.map((item, idx) => (
                        <button key={idx} type="button" onClick={() => { setNewDestination(`${item.name}${item.admin1 ? `, ${item.admin1}` : ''}, ${item.country}`); setDestinationSuggestions([]); }} className="w-full text-left px-3 py-2 text-[11px] text-slate-200 hover:bg-teal-950 flex items-center justify-between border-b border-slate-800 cursor-pointer">
                          <span className="font-bold">{item.name} {item.admin1 ? `- ${item.admin1}` : ''}</span>
                        </button>
                      ))}
                    </div>
      ...
      {/* 1. EN-TÊTE ÉPURÉ */}
      <div className="flex items-center justify-between p-3.5 bg-slate-900/90 backdrop-blur-md rounded-2xl border border-slate-800 shadow-sm">
        <div className="flex items-center space-x-3">
          <div className="p-2 rounded-xl bg-sky-500/10 text-sky-400">
            <Navigation className="w-4 h-4" />
          </div>
          <div>
            <h1 className="text-sm font-bold tracking-tight text-white">Assistant Mobilité</h1>
            <p className="text-[11px] text-slate-400">{currentTemp}°C • {weatherCond}</p>
          </div>
        </div>
        <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-slate-800/80 border border-slate-700/50 text-[10px] font-semibold text-slate-300">
      ...
      <div className="flex items-center gap-2 overflow-x-auto pb-1 no-scrollbar">
        {trips.map((trip) => {
          ...
          return (
            <div
              ...
              className={`px-3 py-1.5 rounded-xl border text-xs font-medium flex items-center gap-2.5 transition-all whitespace-nowrap cursor-pointer ${
                isSelected
        ...
        <button
          ...
          className="px-3 py-1.5 rounded-xl bg-slate-900 border border-dashed border-slate-700 text-sky-400 hover:text-white font-semibold flex items-center gap-1 cursor-pointer"
        >
      ...
      {/* 3. SÉLECTEUR DE MODE, IA & TEMPS ESTIMÉ */}
      <div className="bg-slate-900/90 rounded-2xl p-3 border border-slate-800 shadow-md space-y-2.5">
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 bg-slate-950 p-1 rounded-xl border border-slate-800 flex-1">
            <button
              ...
              className={`flex-1 py-1.5 px-3 rounded-lg font-semibold text-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer ${
                activeMode === 'car' ? 'bg-sky-500 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 py-1.5 px-3 rounded-lg font-semibold text-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer ${
                activeMode === 'bus' ? 'bg-teal-500 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'
              }`}
            >
          ...
          <button
            ...
            className="p-2 rounded-xl bg-indigo-600/30 hover:bg-indigo-600/50 border border-indigo-500/30 text-indigo-300 flex items-center justify-center transition-all cursor-pointer"
          >
            <Sparkles className="w-4 h-4 text-indigo-400" />
          </button>
        </div>

        <div className="flex items-center justify-between px-3 py-1.5 rounded-xl bg-slate-950/60 border border-slate-800 text-[11px]">
          <span className="text-slate-400">Temps estimé ({activeMode === 'car' ? 'Voiture' : 'Transports'})</span>
      ...
      <div className="bg-slate-900/90 rounded-2xl p-4 border border-slate-800 shadow-md space-y-3">
        <div className="flex items-center justify-between border-b border-slate-800/80 pb-2.5">
          <span className="text-xs font-bold uppercase tracking-wider text-slate-300 flex items-center gap-2">
            <Crosshair className="w-3.5 h-3.5 text-sky-400" /> Itinéraire à la volée
          </span>
          {activeMode === 'bus' && (
            <button
              onClick={handleFindNearbyBusStops}
              disabled={isLoadingBusStops}
              className="text-[10px] font-semibold text-teal-400 hover:underline flex items-center gap-1 cursor-pointer"
            >
        ...
        <div className="relative flex items-center gap-3">
          <div className="flex flex-col items-center justify-center py-1.5 pl-1">
            ...
            <div className="w-0.5 h-7 bg-slate-700 my-1" />
          ...
          <div className="flex-1 space-y-2">
            ...
            <div className="relative flex items-center">
              <input
                ...
                className="w-full bg-slate-950/80 border border-slate-800 rounded-xl px-3 py-2 pr-9 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-sky-500"
              />
              <button onClick={() => handleGetGpsPosition('origin')} title="Ma position GPS" className="absolute right-2.5 p-1 text-slate-400 hover:text-sky-400 cursor-pointer">
                ...
                <div className="absolute left-0 right-0 top-full mt-1 bg-[#050811] border border-sky-400 rounded-xl shadow-2xl z-[999999] overflow-hidden max-h-40 overflow-y-auto">
                  {flyOriginSuggestions.map((item, idx) => (
                    <button key={idx} type="button" onClick={() => { 
                      ...
                      setIsUsingFlyMode(true); 
                    }} className="w-full text-left px-3 py-2 text-[11px] text-slate-200 hover:bg-sky-950 flex items-center justify-between border-b border-slate-800 cursor-pointer">
                      <span className="font-bold">{item.name} {item.admin1 ? `- ${item.admin1}` : ''}</span>
                    </button>
                  ))}
                </div>
            ...
            <div className="relative flex items-center">
              <input
                ...
                className="w-full bg-slate-950/80 border border-slate-800 rounded-xl px-3 py-2 pr-9 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-teal-500"
              />
              <button onClick={() => handleGetGpsPosition('destination')} title="Ma position GPS" className="absolute right-2.5 p-1 text-slate-400 hover:text-teal-400 cursor-pointer">
                ...
                <div className="absolute left-0 right-0 top-full mt-1 bg-[#050811] border border-teal-400 rounded-xl shadow-2xl z-[999999] overflow-hidden max-h-40 overflow-y-auto">
                  {flyDestinationSuggestions.map((item, idx) => (
                    <button key={idx} type="button" onClick={() => { 
                      ...
                      setIsUsingFlyMode(true); 
                    }} className="w-full text-left px-3 py-2 text-[11px] text-slate-200 hover:bg-teal-950 flex items-center justify-between border-b border-slate-800 cursor-pointer">
                      <span className="font-bold">{item.name} {item.admin1 ? `- ${item.admin1}` : ''}</span>
                    </button>
                  ))}
                </div>
          ...
          <button
            ...
            className="p-2.5 rounded-xl bg-slate-800/80 hover:bg-slate-700 text-slate-300 border border-slate-700/50 cursor-pointer"
          >
        ...
        {busStopError && activeMode === 'bus' && (
          <div className="p-2.5 rounded-xl bg-rose-950/80 border border-rose-500/60 text-rose-200 text-[10px] font-mono flex items-center justify-between shadow-lg">
            <span className="break-all">⚠️ {busStopError}</span>
            <button onClick={() => setBusStopError(null)} className="text-rose-400 hover:text-white font-bold ml-2 flex-shrink-0 cursor-pointer">✕</button>
          </div>
        )}

        {/* Arrêts de bus à proximité */}
        {activeMode === 'bus' && nearbyBusStops.length > 0 && (
          <div className="pt-2.5 space-y-2 border-t border-slate-800/60 animate-fade-in">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                <Bus className="w-3 h-3 text-teal-400" /> Arrêts à proximité ({nearbyBusStops.length})
              </span>
              <span className="text-[9px] text-slate-500 font-mono">Rayon 1 km</span>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`p-2.5 rounded-xl border text-left transition-all cursor-pointer flex items-center justify-between group ${
                      isClosest 
                        ? 'bg-teal-950/20 border-teal-500/40 hover:border-teal-400' 
                        : 'bg-slate-950/60 border-slate-800 hover:border-slate-700 hover:bg-slate-900/60'
                    }`}
                  >
                    <div className="flex items-center gap-2.5 truncate">
                      <div className={`p-1.5 rounded-lg flex-shrink-0 ${isClosest ? 'bg-teal-500/20 text-teal-300' : 'bg-slate-900 text-slate-400'}`}>
                        <MapPin className="w-3 h-3" />
                      </div>
                      <div className="truncate">
                        <div className="font-semibold text-slate-200 truncate group-hover:text-white text-[11px]">
                          {stop.name}
                        </div>
                        {stop.routes && (
                    ...
                    <div className="text-right flex-shrink-0 ml-2">
                      <span className={`text-[10px] font-mono font-bold px-1.5 py-0.5 rounded ${
                        isClosest ? 'bg-teal-500/20 text-teal-300' : 'bg-slate-900 text-slate-400'
                      }`}>
        ...
        {activeMode === 'car' && (
          <div className="pt-2 space-y-2 border-t border-slate-800/60">
            <div className="flex items-center justify-between text-[10px] text-slate-400 font-semibold uppercase tracking-wider">
              <span className="flex items-center gap-1 text-sky-400"><Fuel className="w-3 h-3" /> Prix Carburants (ACL)</span>
              <button onClick={loadFuelPrices} className="hover:text-white flex items-center gap-1"><RefreshCw className={`w-3 h-3 ${isRefreshingFuel ? 'animate-spin' : ''}`} /></button>
            </div>
            <div className="grid grid-cols-3 gap-2">
              <div className="p-2 rounded-xl bg-slate-950/60 border border-slate-800 text-center">
                ...
                <span className="text-xs font-bold text-white mt-0.5 block">{fuelPrices.super95}</span>
              </div>
              <div className="p-2 rounded-xl bg-slate-950/60 border border-slate-800 text-center">
                <span className="text-[9px] text-slate-400 font-bold block">SUPER 98</span>
                <span className="text-xs font-bold text-white mt-0.5 block">{fuelPrices.super98}</span>
              </div>
              <div className="p-2 rounded-xl bg-slate-950/60 border border-slate-800 text-center">
                <span className="text-[9px] text-slate-400 font-bold block">DIESEL</span>
                <span className="text-xs font-bold text-white mt-0.5 block">{fuelPrices.diesel}</span>
              </div>
            </div>
          </div>
        )}
      ...
      {/* 6. TRAFIC & DIAGNOSTIC */}
      <div className="bg-slate-900/90 rounded-2xl p-3.5 border border-slate-800 shadow-md space-y-2 text-xs">
        <div className="flex items-center justify-between border-b border-slate-800/80 pb-2">
          <span className="font-bold text-slate-300 flex items-center gap-1.5">
            <ShieldAlert className="w-3.5 h-3.5 text-sky-400" /> Diagnostic Trafic
          </span>
          <span className="text-[10px] font-semibold text-teal-400 flex items-center gap-1">
            <CheckCircle2 className="w-3 h-3" /> Fluide
          </span>
        </div>
